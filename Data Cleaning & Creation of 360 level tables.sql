...
SELECT * FROM Orders_104
select * from [dbo].[Customer_Level_360]

select * from [dbo].[Order_Level_360]

select * from [dbo].[Store_Level_360]

SELECT COUNT(DISTINCT Custid) FROM Customer_Level_360

--Number of Orders
SELECT COUNT(DISTINCT OrderID) AS Number_Of_Orders
FROM Order_Level_360;
...
SELECT AVG(Discount) AS Avg_Discount_Per_Customer
FROM Customer_Level_360;
...
SELECT COUNT(DISTINCT ProductCategory) AS Total_Categories
FROM Order_Level_360;

--Total Stores

SELECT COUNT(DISTINCT StoreID) AS Total_Stores
FROM Store_Level_360;

--Total Locations

SELECT COUNT(DISTINCT CONCAT(seller_city, '-', seller_state)) AS Total_Locations
FROM Store_Level_360;
...
SELECT COUNT(DISTINCT PaymentType) AS Total_Payment_Methods
FROM Order_Level_360;

--Percentage of Profit

SELECT (SUM(Profit) * 100.0) / SUM([Total_cost]) AS Profit_Percentage
FROM [dbo].[Order_Level_360]

--Most Frequent Product Purchased

SELECT TOP 1 ProductID, COUNT(ProductID) AS Frequency
FROM Order_Level_360
GROUP BY ProductID
...
SELECT TOP 1 StoreID, SUM(Total_Profit) AS Total_Profit
FROM Store_Level_360
GROUP BY StoreID
...
SELECT AVG(SatisfactionScore) AS Avg_Customer_Satisfaction
FROM Order_Level_360;

--Customer Retention Rate

SELECT (COUNT(*) * 100.0) / (SELECT COUNT(*) FROM Customer_Level_360) AS Retention_Rate
FROM Customer_Level_360
WHERE Tenure > 1;

-- New Customer Acquisition by Month
SELECT 
    FORMAT(First_Transaction_Date, 'yyyy-MMMM') AS Month,
    COUNT(Custid) AS New_Customers
FROM Customer_Level_360
WHERE First_Transaction_Date IS NOT NULL
GROUP BY 
    FORMAT(First_Transaction_Date, 'yyyy-MMMM'),
...
--How the revenues from new customers on monthly basis

WITH FirstTransactionMonth AS (
    SELECT 
        Custid, 
        MIN(YEAR(OrderDate)) AS First_Year,
        MIN(MONTH(OrderDate)) AS First_Month
    FROM Order_Level_360
    GROUP BY Custid
),
MonthlyRevenue AS (
    SELECT 
        ...
        ROUND(SUM(TotalAmount), 2) AS TotalRevenue  -- Rounded to 2 decimal places
    FROM Order_Level_360
    GROUP BY Custid, YEAR(OrderDate), MONTH(OrderDate)
)
-- Final Query to calculate revenue from new customers only
SELECT 
    LEFT(DATENAME(MONTH, DATEFROMPARTS(MR.Year, MR.Month, 1)), 3) + ' ' + CAST(MR.Year AS VARCHAR) AS YearMonth,  -- Concatenate Month and Year
    -- Revenue from New Customers
    ROUND(SUM(CASE 
        WHEN MR.Year = FT.First_Year AND MR.Month = FT.First_Month THEN MR.TotalRevenue
        ELSE 0 
    END), 2) AS New_Customer_Revenue  -- Rounded to 2 decimal places
FROM MonthlyRevenue MR
JOIN FirstTransactionMonth FT
ON MR.Custid = FT.Custid
GROUP BY MR.Year, MR.Month
ORDER BY MR.Year, MR.Month;


-- Sales, Quantity & Profit Trends by Category:

SELECT 
    YEAR(OrderDate) AS Year, 
    MONTH(OrderDate) AS Month,
    LEFT(DATENAME(MONTH, DATEFROMPARTS(YEAR(OrderDate), MONTH(OrderDate), 1)), 3) AS MonthName,
    ProductCategory,
    SUM(TotalAmount) AS TotalSales,
    SUM(Quantity) AS TotalQuantity,
    SUM(TotalAmount - [Total_Cost]) AS TotalProfit  
FROM Order_Level_360
GROUP BY 
    YEAR(OrderDate), 
    MONTH(OrderDate),
...
--  Sales and Quantity Trends by Region:
...
-- Sales and Quantity Trends by Store:
...
-- Sales and Quantity Trends by Channel:

SELECT 
    channel,
    SUM(TotalAmount) AS TotalSales,
    SUM(Quantity) AS TotalQuantity,
    SUM([Profit]) AS TotalProfit  -- Replace with 'Profit' column if available
FROM Order_Level_360
GROUP BY 
    channel
ORDER BY TotalSales DESC;
...
SUM(TotalAmount) AS TotalSales,
SUM(Quantity) AS TotalQuantity,
RANK() OVER (PARTITION BY StoreID ORDER BY SUM(TotalAmount) DESC) AS CategoryRank
FROM Order_Level_360
...
ORDER BY TotalSales DESC, StoreID;
...
--25  top 10 most expensive products sorted by price and their contribution to sales
SELECT TOP 10 ProductID, ProductCategory, 
MAX([cost per unit]) AS MaxPrice, 
SUM(TotalAmount) AS TotalSales,
(SUM(TotalAmount) * 100.0 / (SELECT SUM(TotalAmount) FROM Order_Level_360)) AS ContributionToSalesPercentage
FROM Order_Level_360
GROUP BY ProductID, ProductCategory
ORDER BY MaxPrice DESC;

--26 gender wise contibution 
SELECT Gender,
SUM(TotalAmount) AS TotalSales,
SUM(Quantity) AS TotalQuantity
FROM Order_Level_360
...
WITH RFM AS (
    SELECT
        Custid, Last_Transaction_Date, Frequency, Monetary     
    FROM
        Customer_Level_360
    GROUP BY
        Custid, Last_Transaction_Date, Frequency,Monetary
...
CurrentDate AS (
    SELECT CAST('2023-12-31' AS DATE) AS END_DATE  -- Define a reference date (e.g., end of analysis period)
)
SELECT
    RFM.Custid,
    DATEDIFF(DAY, Last_Transaction_Date, END_DATE) AS Recency -- Days since last purchase
FROM
    RFM
CROSS JOIN
    CurrentDate;
...
WITH RFM_Scored AS (
    SELECT
        Custid,
        Recency,
        NTILE(4) OVER (ORDER BY Recency ASC) AS Recency_Score,  -- Lower recency = Higher score
        Frequency,
        NTILE(4) OVER (ORDER BY Frequency DESC) AS Frequency_Score, -- Higher frequency = Higher score
        Monetary,
        NTILE(4) OVER (ORDER BY Monetary DESC) AS Monetary_Score   -- Higher monetary = Higher score
    FROM (
	;WITH RFM AS (
	SELECT
        Custid, Last_Transaction_Date, Frequency, Monetary       
...
CurrentDate AS (
    SELECT CAST('2023-12-31' AS DATE) AS END_DATE
),
RawRFM AS (
    SELECT
        RFM.Custid,
        DATEDIFF(DAY, RFM.Last_Transaction_Date, CurrentDate.END_DATE) AS Recency,
        RFM.Frequency,
        RFM.Monetary
    FROM
        RFM
...
SELECT
	...
	CASE
        WHEN (Recency_Score + Frequency_Score + Monetary_Score) BETWEEN 3 AND 6 THEN 'At Risk Customers'
        WHEN (Recency_Score + Frequency_Score + Monetary_Score) BETWEEN 7 AND 9 THEN 'Loyal Customers'
        WHEN (Recency_Score + Frequency_Score + Monetary_Score) BETWEEN 10 AND 12 THEN 'Best Customers'
        ELSE 'Undefined'
